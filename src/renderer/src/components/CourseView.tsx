...
const CourseView: React.FC<CourseViewProps> = ({ courseId, onBack }) => {
  ...
  useEffect(() => {
    ...
    try {
      cleanup = window.api.onCourseUpdated((updatedCourseId) => {
  ...
  const handleRescanCourse = async () => {
    setIsRescanning(true)
    try {
      await window.api.refreshCourse(courseId)
      await loadCourse()
      setShowSettings(false)
    } catch (e) {
      console.error('Failed to rescan course:', e)
    } finally {
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden text-slate-200 z-10 relative">
      <nav className="h-16 px-6 glass-panel border-b border-white/5 flex items-center justify-between shrink-0 sticky top-0 bg-surface-800/80 z-20">
          ...
          <div className="flex items-center">
            ...
            <div className="flex items-center gap-3 ml-4">
              <div className="w-7 h-7 rounded bg-gradient-to-br from-brand-500 to-purple-600 flex items-center justify-center shadow-lg shadow-brand-500/20 shrink-0">
                 {course.icon ? <span className="text-xs">{course.icon}</span> : <LayoutGrid className="w-3.5 h-3.5 text-white" />}
              </div>
                  ...
                  {isEditingTitle ? (
                    <div className="flex items-center gap-2">
                      <div className="relative" ref={emojiPickerRef}>
                        <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="bg-surface-900 border border-brand-500 rounded px-2 py-0.5 text-xs font-bold text-white focus:outline-none min-w-[32px] h-7 flex items-center justify-center hover:bg-surface-800 transition-colors">{editIconValue || '✨'}</button>
                        {showEmojiPicker && (
                          <div className="absolute top-full mt-2 left-0 z-50 shadow-2xl">
                            <EmojiPicker theme={'dark' as any} emojiStyle={EmojiStyle.NATIVE} onEmojiClick={(emojiData) => { setEditIconValue(emojiData.emoji); setShowEmojiPicker(false) }} />
                          </div>
                        )}
                      </div>
            ...
            {showSettings && (
              <div className="absolute right-0 mt-3 w-64 bg-surface-800 border border-white/10 rounded-xl shadow-modal overflow-hidden z-30 animate-fade-in origin-top-right">
                ...
                <div className="p-1">
                  ...
                  <button onClick={handleRescanCourse} disabled={isRescanning} className="w-full px-4 py-2.5 text-left text-sm text-slate-300 hover:bg-brand-500/10 hover:text-brand-300 flex items-center gap-3 rounded-lg disabled:opacity-50">
                    <RefreshCw className={`w-4 h-4 ${isRescanning ? 'animate-spin' : ''}`} /> Rescan Folder
                  </button>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative bg-[#050505]">
             ...
             <div className="w-full bg-black shrink-0 lg:flex-1 flex items-center justify-center">
               {activeVideo ? (
                 ...
                 <div className="w-full h-[300px] md:h-[450px] lg:h-full flex flex-col items-center justify-center text-slate-500 bg-surface-900/40 backdrop-blur-sm">
                   ...
                   <p className="text-lg font-medium text-slate-400 text-center px-4">Select a video to start learning</p>
                 </div>
               )}
             </div>
           ...
           <div className="absolute inset-0 bg-black/80 z-50 flex flex-col items-center justify-center animate-fade-in backdrop-blur-sm">
             ...
             <div className="flex flex-col items-center bg-surface-800/80 p-8 rounded-3xl border border-white/10 shadow-2xl">
               ...
               <div className="flex gap-4">
                 ...
                 <button onClick={() => { const nextVid = getNextVideo(); if (nextVid) { setActiveVideo(nextVid); setCompletionState(null) } }} className="px-6 py-3 rounded-xl bg-brand-500 hover:bg-brand-400 text-white font-bold transition-colors flex items-center gap-2">Play Next <Play className="w-4 h-4 fill-current" /></button>
               </div>
             </div>
           </div>
        )}
        ...
        {completionState === 'final' && (
           <div className="absolute inset-0 bg-black/80 z-50 flex flex-col items-center justify-center animate-fade-in backdrop-blur-sm">
             ...
             <p className="text-slate-300 text-xl mb-8 max-w-lg text-center leading-relaxed">You've reached the end of this course.</p>
             <div className="w-full max-w-2xl bg-surface-800/80 p-6 rounded-3xl border border-white/10 shadow-2xl mx-4">
               <textarea id="final-note" placeholder="Write your final review..." className="w-full bg-surface-900 border border-white/5 rounded-2xl p-5 text-base text-slate-200 focus:outline-none focus:border-brand-500 min-h-[160px] mb-6 resize-none shadow-inner" />
